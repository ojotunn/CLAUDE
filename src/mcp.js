...
import { z } from 'zod';
import { APP_NAME, VERSION, CHAIN, LIMITS, PUBLIC_URL, VENUE } from './config.js';
...
const INSTRUCTIONS = ARGUS
  ? `${APP_NAME} launches tokens on Argus (Arc, the Circle chain where gas is USDC) from a conversation. Argus has no bonding curve: the token is born in a Uniswap v4 pool with a fixed buy tax and sell tax (0-10% each, at least one above 0, fixed forever) that is split between the creator, buy back & burn and liquidity (a holders share paid in USDC exists on Argus but only launchers registered by Argus can use it; regular wallets cannot). Argus keeps 10% of the tax. Supply is 1,000,000,000; the pool opens at a $2,500 market cap; at $45,000 the liquidity locks for good and trading continues in the same pool. There is no launch fee; the creator pays gas in USDC. ${APP_NAME} never holds the user's keys: the user signs in their own wallet through a link.

How to use it:
1. Turn what the user described into preview_launch (name, ticker, description, optional dev buy in USDC, optional buy/sell tax and tax split). Show the user the terms it returns: name, ticker, supply, taxes and split, dev buy and share of supply, total USDC cost, warnings.
2. Only after the user confirms, call prepare_launch with the same fields. It returns a signing link. Give the user that link; they connect their wallet there, see the exact contract address, and sign (an approval first if there is a dev buy, then the launch).
...
4. token_info reports price in USD, market cap, taxes, split and progress to the $45,000 bond for any Argus token. prepare_buy returns a signing link to buy a token through the Uniswap v4 pool (the first buy from a wallet takes up to three signatures: two approvals, then the swap).
5. Agents: on Argus the creator of a token cannot be changed, so an agent has to launch its own token. Call prepare_launch with withAgent: true (plus vibe, avatar, rules). The agent gets a wallet of its own; the user sends the launch money (dev buy plus a few USDC of gas) to that wallet on the signing page; the agent launches the token and is its creator. From then on it collects the creator share of the tax and, on its own, buys back and burns, airdrops recent buyers (optionally only those who never sold), pays the owner a salary, runs raffles, buys dips, keeps a gas reserve, and posts about it on its public page (and on X / Telegram if the owner connected them). No approvals per action. set_agent_rules changes the split and the posting habits. agent_status reports what it did. ask_agent lets anyone ask the token something. release_agent explains how the owner takes the balance back (the agent stays the creator on-chain and forwards every fee to the owner from then on). attach_agent does not work on Argus (nothing to hand over).

Facts: supply is fixed by Argus per launch; dev buys are capped at ${(LIMITS.maxDevBuyBps / 100).toFixed(0)}% of supply; the wallet that signs (or the agent wallet) is the creator and receives the creator share of the tax; amounts are in USDC. Network: ${CHAIN.name} (chain id ${CHAIN.id}).`
  : `${APP_NAME} launches tokens on pons (Robinhood Chain, pons v2 bonding curve) from a conversation. It never holds keys: the user signs in their own wallet through a link.
...
const ruleShape = {
  ...
  whaleAmount: z.string().optional().describe(`A single buy at or above this ${unit} amount triggers a post. Default "${chain.AGENT_DEFAULTS.whale}".`),
};
...
const launchShape = {
  ...
  devBuy: z.string().optional().describe(`${unit} the creator spends buying in the same transaction as the launch, e.g. "${ARGUS ? '20' : '0.05'}". Omit or "0" for none.`),
  wallet: z.string().optional().describe('The wallet that will sign, if the user already told you. Enables exact contract address prediction.'),
  ...(ARGUS ? {
    buyTaxPct: z.number().optional().describe('Tax on every buy, in percent (0-10). Default 3. Fixed forever.'),
    sellTaxPct: z.number().optional().describe('Tax on every sell, in percent (0-10). Default 3. Fixed forever. At least one of buy/sell tax must be above 0.'),
    creatorShare: z.number().optional().describe('Share of the tax that goes to the creator, in percent. Default 100. creatorShare + burnShare + holdersShare + liquidityShare must equal 100.'),
    burnShare: z.number().optional().describe('Share of the tax used to buy back and burn the token, in percent. Default 0.'),
    holdersShare: z.number().optional().describe('Share of the tax paid to holders in USDC. Only launchers registered by Argus can use it; for a regular wallet the preview fails with a clear error. Default 0.'),
    liquidityShare: z.number().optional().describe('Share of the tax added to the pool as liquidity. Default 0.'),
    withAgent: z.boolean().optional().describe('Launch through an agent: the agent gets its own wallet, the user funds it on the signing page, the agent launches and keeps the creator share of the tax, then runs itself (buy back & burn, airdrops, salary, posts).'),
    vibe: z.string().optional().describe('With withAgent: one line of personality for the agent\'s posts'),
    avatar: z.string().optional().describe('With withAgent: https URL of a profile picture for the agent'),
    ...ruleShape,
  } : {
    discord: z.string().optional().describe('Discord URL'),
    farcaster: z.string().optional().describe('Farcaster URL or handle'),
    devBuyEth: z.string().optional().describe('Same as devBuy (ETH). Kept for older prompts.'),
...
export function createMcpServer() {
  ...
  server.registerTool('launch_terms', {
    title: `Current ${chain.NAME} launch terms`,
    description: ARGUS
      ? 'Reads the live Argus terms on Arc: supply, opening and bond market caps, maximum tax, how the tax can be split, dev buy cap, contracts.'
      : 'Reads the live pons v2 launch terms: launch fee, fixed supply, maximum creator tax, whether launches are open to everyone, graduation threshold.',
    inputSchema: {},
  }, run(async () => ok(chain.termsSummary(await chain.protocolTerms()))));
  ...
  }, run(async (a) => ok(await launches.preview(launches.normalizeLaunchRequest(a)), 'Launch preview (nothing signed yet):')));

  server.registerTool('prepare_launch', {
    title: 'Prepare a launch for signing',
    description: `Stores the launch and returns a signing link. Call only after the user confirmed the preview. The user opens the link, connects their wallet, sees the contract address and signs.${ARGUS ? ' With withAgent, the link asks the user to fund the agent wallet instead; the agent then launches.' : ''}`,
    inputSchema: launchShape,
  }, run(async (a) => {
    const rec = await launches.prepare(launches.normalizeLaunchRequest(a));
    const text = rec.kind === 'agent-launch'
      ? `Ready. The agent wallet is ${rec.agent.address}. Send the user this link so they fund it and the agent launches: ${rec.url}. Agent page: ${rec.agent.page}`
      : `Ready to sign. Send the user this link: ${rec.url}`;
    return ok({ id: rec.id, kind: rec.kind, url: rec.url, expiresAt: rec.expiresAt, summary: rec.summary, agent: rec.agent }, text);
  }));

  server.registerTool('launch_status', {
    title: 'Launch status',
    description: 'Status of a prepared launch or buy by id: awaiting_wallet, needs_funds, ready, submitted, launching (agent launch), live (token deployed), done (buy filled), failed, expired. Returns the contract address once live.',
    ...
    return ok({
      ...
      token: r.token, curve: r.curve, tokensOut: r.tokensOut, txHash: r.launchTxHash || r.txHash, error: r.error,
      funding: r.funding, predicted: r.predicted, agent: r.agent, links: r.links, warnings: r.warnings,
    });
  }));

  server.registerTool('token_info', {
    title: `${chain.NAME} token info`,
    description: ARGUS
      ? 'Live state of an Argus token: name, ticker, price in USD, market cap, buy/sell tax and split, opening tax, progress to the $45,000 bond, whether liquidity is locked.'
      : 'Live state of a pons v2 token: name, ticker, price in ETH, market cap, ETH raised, graduation progress, phase, creator tax.',
    ...
    if (!info) throw new launches.UserError(`this address is not a ${chain.NAME} launch on this network`, 'NOT_VENUE_TOKEN');
  ...
  server.registerTool('prepare_buy', {
    title: ARGUS ? 'Prepare a buy on the Uniswap v4 pool' : 'Prepare a buy on the bonding curve',
    description: ARGUS
      ? 'Quotes buying an Argus token with USDC through its Uniswap v4 pool and returns a signing link. Only after the user confirmed the amount.'
      : 'Simulates buying a pons v2 token that is still on its bonding curve and returns a signing link. Only after the user confirmed the amount.',
    inputSchema: {
      token: z.string().describe('Token contract address (0x...)'),
      amount: z.string().describe(`${unit} to spend, e.g. "${ARGUS ? '10' : '0.01'}"`),
      ...(ARGUS ? {} : { ethAmount: z.string().optional().describe('Same as amount (ETH). Kept for older prompts.') }),
  ...
  server.registerTool('attach_agent', {
    ...
    description: ARGUS
      ? 'Not available on Argus: the creator of a token cannot be changed there, so an agent must launch its own token. Use prepare_launch with withAgent: true. This tool only explains that.'
      : 'Creates an agent wallet for a live pons v2 token and returns a link where the creator (current creator-fee recipient) signs once to hand the creator fees to the agent. After that the agent acts on its own: collect fees, buy back and burn, airdrop recent buyers, keep a gas reserve, post about it. Call only after the user asked for an agent.',
    inputSchema: {
      token: z.string().describe('Token contract address (0x...)'),
  ...
  server.registerTool('release_agent', {
    ...
    description: ARGUS
      ? 'Explains how the owner takes the agent balance and tokens back. The agent stays the creator on-chain (Argus cannot change it) and forwards every fee it collects to the owner from then on. It requires a wallet signature on the agent page, so this tool only returns the link and the steps.'
      : 'Explains how the creator takes the creator fees and the agent balance back. It requires a wallet signature on the agent page, so this tool only returns the link and the steps.',
    inputSchema: { token: z.string().describe('Token contract address (0x...)') },
  }, run(async ({ token }) => {
    ...
    return ok({ page: `${PUBLIC_URL}/t/${rec.token}`, status: rec.status },
      `Open ${PUBLIC_URL}/t/${rec.token}, press "Manage" and sign the login message with the owner wallet (${rec.creator || 'the wallet that funded the launch'}), then press "Release agent". ${ARGUS ? 'The agent sends its balance and tokens back and keeps forwarding every fee it collects to that wallet.' : 'The agent hands the fee recipient role and its balance back to that wallet.'}`);
  }));
