...
test('launch_terms reads the live protocol terms', async () => {
  ...
  assert.match(data.launchFee, /^\d+(\.\d+)?$/);
  assert.equal(data.unit, 'ETH');
...
test('preview_launch simulates a launch with a dev buy', async () => {
  ...
  assert.equal(data.cost.devBuy, '0.01');
  assert.equal(data.cost.unit, 'ETH');
  assert.equal(Number(data.cost.total), Number(data.cost.launchFee) + 0.01);
...
test('preview_launch clamps an oversized dev buy', async () => {
  ...
  assert.ok(Number(data.devBuy.amount) < 2);
...
test('signing page is served and binds a wallet with a predicted address', async () => {
  ...
  assert.equal(BigInt(rec.tx.value), BigInt(Math.round((Number(rec.terms.launchFee) + 0.02) * 1e18)));
  assert.deepEqual(rec.pre, [], 'pons needs no approval step');
...
test('token_info reads a real pons v2 token', async () => {
  ...
  assert.equal(data.unit, 'ETH');
  assert.equal(typeof data.price, 'number');
});

test('prepare_buy simulates a curve buy and binds', async () => {
  const { data, isError, text } = await callTool('prepare_buy', { token: PONSDROP, amount: '0.001' });
...
test('HTTP guards: GET /mcp is 405, unknown launch is 404, recent list is empty', async () => {
  ...
  assert.match(homeHtml, /Add it to Claude/);
  assert.match(homeHtml, /Robinhood/, 'home is rendered for the pons venue');
  assert.ok(!/\{\{#|\{\{\/|\{\{[A-Z_]+\}\}/.test(homeHtml), 'no template markers leak into the page');
});
...
test('agent page and public API expose no secrets; creator endpoints need a signature', async () => {
  ...
  assert.equal(v.permanentRole, false);
